...
CREATE TABLE `lsv_plans` (
  ...
  `interval` enum('H','D','W','M','Y') COLLATE utf8_unicode_ci NOT NULL COMMENT 'H=Hours (1 to 24) D=Days (1 to 90) | W=Weeks (1 to 52) | M=Months (1 to 24) | Y=Years (1 to 5)',
...
CREATE TABLE `lsv_subscriptions` (
  ...
  `agent_id` int(11) NOT NULL DEFAULT 0 COMMENT 'foreign key of "agents" table',
  `plan_id` int(5) DEFAULT NULL COMMENT 'foreign key of "plans" table',
  `payment_method` enum('paypal', 'stripe', 'authorizenet', 'manual') COLLATE utf8_unicode_ci NOT NULL DEFAULT 'paypal',
  `payment_id` varchar(100) COLLATE utf8_unicode_ci NOT NULL,
  `txn_id` varchar(100) COLLATE utf8_unicode_ci DEFAULT NULL,
  `subscr_interval` char(1) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'H=Hours | D=Days | W=Weeks | M=Months | Y=Years',
...
ALTER TABLE `lsv_agents` ADD `openai_key` VARCHAR(255) NULL AFTER `recovery_token`, ADD `openai_model` VARCHAR(50) NULL AFTER `openai_key`, ADD `heygen_key` VARCHAR(255) NULL AFTER `openai_model`;
